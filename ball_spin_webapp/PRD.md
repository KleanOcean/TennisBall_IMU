...
│  │ ax/ay/az 三轴折线图     │ gx/gy/gz 三轴折线图       │        │
│  ├─────────────────────────┼───────────────────────────┤        │
│  │ G-FORCE                 │ ORIENTATION               │        │
│  │ 圆弧仪表（实时G值）     │ Pitch/Roll/Yaw + 水平仪   │        │
│  └─────────────────────────┴───────────────────────────┘        │
│  （详见 Feature #10：原始传感器数据可视化）                        │
└─────────────────────────────────────────────────────────────────┘
```
...
- **DASHBOARD 三栏布局**：
  ...
  - 右栏（35%）：实时折线图 + 击球时间线
- **移动端**：三栏改为纵向堆叠（球 → RPM → 图表 → 时间线 → RAW DATA）
- **RAW DATA 可折叠区域**：位于时间线下方，默认折叠，展开后显示 2x2 网格（加速度折线图、陀螺仪折线图、G 力仪表、姿态角度），详见 Feature #10
...
按下按钮唤醒后，播放约 1.2 秒的日出动画，模拟太阳从海平面升起：

```
  阶段 1（0-0.3s）        阶段 2（0.3-0.7s）       阶段 3（0.7-1.2s）
┌──────────────────┐   ┌──────────────────┐   ┌──────────────────┐
│                  │   │    ·  · ·        │   │   · · · · · ·    │
│                  │   │   · · · · ·      │   │  ·    ☀️     ·   │
│                  │   │  ·         ·     │   │  · 网球=太阳  ·  │
...
│ ░░░░░░░░░░░░░░░ │   │ ░░░░░░░░░░░░░░░ │   │  1250 RPM        │
└──────────────────┘   └──────────────────┘   └──────────────────┘
   深蓝色海面等待         半圆升出海面            太阳升至中央
   底部海水 + 星空        光线向上扩散            淡出为正常画面
```

**视觉效果**：
- **背景渐变**：从极深藏青色（#000820）渐变到深蓝（#0a1040），模拟黎明天空
- **海面**：屏幕下半部分为深色海水（与入睡海水同色系），水面线有微亮的青色高光
- **太阳=荧光黄绿色圆**：从海面下方缓缓升起
  - 颜色使用品牌色荧光黄绿（0xCE40），与网球同色，暗示"太阳就是网球"
  - 半径从 15px 开始，升到中央时为 BALL_R（30px），最终变为正常网球画面
- **光晕扩散**：太阳周围有同心圆光芒，从中心向外扩散（荧光黄绿色渐淡）
- **过渡**：动画最后一帧淡入正常的网球 + RPM + WiFi 信息画面
...
## Feature #10: 原始传感器数据可视化 (Raw Sensor Data Visualization)

### 概述

在 DASHBOARD 标签页现有布局下方新增可折叠的"RAW DATA"区域，展示 IMU 传感器原始数据的实时可视化，帮助教练深入分析击球力度、旋转轴稳定性和球体姿态。

### 布局设计

- 位于现有 DASHBOARD 三栏布局和 SHOT TIMELINE 下方
- 默认折叠状态，点击 "▼ RAW DATA" 按钮展开
- 展开后显示 2x2 网格布局（桌面端），移动端纵向堆叠
- 折叠状态下不渲染图表（节省 CPU 和电量）

### 模块 1：三轴加速度实时折线图 (ACCELEROMETER)

- 三条叠加折线：ax（红 #FF6B6B）、ay（绿 #4ECDC4）、az（蓝 #6B9FFF）
- Canvas 渲染，滚动窗口 200 个数据点（~4 秒 @50Hz）
- Y 轴：自动缩放，单位 g
- 深色背景（#0a0e27），网格线（#1a2040）
- 用途：清晰观察击球瞬间的加速度脉冲尖峰（>4g），分析挥拍轨迹

### 模块 2：三轴陀螺仪实时折线图 (GYROSCOPE)

- 三条叠加折线：gx（红 #FF6B6B）、gy（绿 #4ECDC4）、gz（蓝 #6B9FFF）
- Canvas 渲染，滚动窗口 200 个数据点
- Y 轴：自动缩放，单位 °/s
- 用途：直观判断旋转主导轴（gx 大=上旋/下旋，gy/gz 大=侧旋/切削），分析旋转轴稳定性

### 模块 3：G 力峰值仪表 (G-FORCE)

- 类似 RPM 仪表的圆弧仪表，显示实时加速度合力 √(ax²+ay²+az²)
- 弧形仪表：灰色底轨 + 荧光黄绿色填充弧
- 中央大号数字显示当前 G 值
- 下方显示最近一次击球的峰值 G（保留直到下次击球）
- 最大刻度：10g
- 用途：量化击球力度

### 模块 4：姿态角度显示 (ORIENTATION)

- 从四元数转换为欧拉角（Pitch / Roll / Yaw）
- 三个数值以大号 monospace 字体显示
- 下方绘制简单的水平仪圆形指示器：
  - 外圆 + 十字线参考
  - 小圆点（气泡）根据 Pitch/Roll 偏移
  - 水平时气泡居中
- 用途：了解球的姿态角度
...
- 所有模块遵循 YoTB 深色科技风配色
- 卡片背景：#111633，边框：#1a2040
- 图表刷新率：~10fps（每 100ms 一帧），比主界面低以节省性能
- 折叠按钮样式：与导航栏一致的胶囊形按钮

### 响应式适配

- 桌面端（>768px）：2x2 网格（加速度+陀螺仪 上排，G力+姿态 下排）
- 移动端（≤768px）：四个模块纵向堆叠

### PROGMEM 约束

- 新增代码需控制在 ~20KB 以内（当前已用 ~24KB，总限制 ~50KB）
- 四个 Canvas 仅在 RAW DATA 展开时渲染
