...
- 使用 Canvas 2D API 渲染 **线框（wireframe）网球模型**
- 绘制经度线（每 30° 一条）和纬度线（每 30° 一条）构成的线框球体
- 网球缝线曲线以更亮、更粗的荧光黄绿色线框绘制
- 所有线条颜色统一为荧光黄绿色（#C8D820）
- 球体外围添加霓虹辉光/泛光效果（CSS box-shadow 或 Canvas glow）
- 深色背景（#0a0e27）衬托绿色线框，视觉冲击力强
- 基于四元数数据实时旋转，与物理设备姿态同步
