...
```
    ...
    ┌────────┴─────────────────────────────────┐
    │                                          │
    │  通路 A：直连 WebSocket（实时）            │
    │  浏览器 ──ws://192.168.4.1:81──► ATOM S3  │
    │  · 球体四元数实时旋转                      │
    │  · RPM 仪表实时更新                        │
    │  · 滚动 RPM 折线图                         │
    │  · 击球事件即时推送                         │
...
5. 仪表盘加载后，浏览器**直接**与 `ws://192.168.4.1:81` 建立 WebSocket 连接，获取实时数据流（四元数、RPM、击球事件）
6. 仪表盘同时通过 REST API 从 Observer 后端获取历史会话、分析统计等离线数据
7. 手机可通过浏览器访问笔记本 IP 的 8080 端口查看仪表盘
8. 训练结束按 Ctrl+C 停止，仪表盘仍可查看历史数据（REST API 通路失效，WebSocket 通路也断开，仅剩本地缓存）
...
- **注意**：仪表盘浏览器直连 `ws://192.168.4.1:81` 也占用一个 WebSocket 客户端槽位。典型部署下三个槽位全部占满：
  - observer.py（1 个）— 数据持久化
  - 仪表盘浏览器（1 个）— 实时可视化
  - 手机边缘端 WebApp（1 个）— 球员实时查看
  - 合计 = 3 个（已达上限，无额外槽位）
...
- 后端：Python asyncio + aiohttp（异步 HTTP 服务，提供 REST API 查询历史数据）
- 前端：单页 HTML + CSS + JavaScript + SVG + Canvas（无第三方依赖）
- **混合数据架构**：
  - **实时数据**：前端直连 `ws://192.168.4.1:81` WebSocket，零中间层，延迟 < 50ms
  - **历史数据**：前端通过 REST API 从 Observer 后端查询（会话列表、击球详情、分析统计、导出）
- 端口：localhost:8080
...
- **实时更新**：RPM 数值和仪表弧由 WebSocket 直连推送的数据驱动，每帧（50Hz）刷新

#### 中栏（约 40%）— 3D 线框网球（核心视觉 HERO 元素）

- Canvas 渲染的 **线框球体** — 测地线球体（Geodesic Sphere / Icosphere）
  - 基于正二十面体细分算法生成均匀三角形网格
  - 一次细分产生约 80 个三角面、120 条边
  - 三角形大小均匀分布，无极点汇聚问题（优于经纬线网格）
- 网球缝线曲线以荧光黄绿色（#C8D820）高亮渲染（更亮、更粗）
- 每条边根据深度（z 值）调节透明度：前面亮（0.8-1.0），背面暗（0.1-0.15）
- 球体外围 **多层辉光效果**：内层高亮（opacity 0.15）+ 外层扩散（opacity 0.06），半径扩展至球体半径的 1.5 倍，营造强烈的霓虹灯管视觉效果
- **球体实时旋转**：通过直连 `ws://192.168.4.1:81` WebSocket 接收四元数数据（qw, qx, qy, qz），驱动球体实时旋转，与边缘端 WebApp 体验一致
- 透视投影：远处线条更暗更细，近处更亮更粗
- Canvas 尺寸：桌面端 400x400
- WebSocket 断开时球体静止，显示最后已知姿态
...
#### 右栏（约 35%）— 实时图表 + 击球时间线（实时）

- **"REAL TIME CHART"** 标题 — **实时滚动 RPM 折线图**
  - Canvas 渲染，数据来自直连 WebSocket 的 RPM 字段
  - 荧光黄绿色折线（#C8D820 / lime green），线下方带渐变填充区域
  - 深色背景（#0a0e27）
  - Y 轴灰色刻度标签，X 轴 MM:SS 时间戳
  - 网格线（#1a2040）
  - 数据窗口：最近 250 个数据点（50Hz 下约 5 秒），自动滚动
  - 每收到一个 WebSocket 数据帧即追加一个点，超出 250 点时左端移除最旧数据
- **"SHOT TIMELINE"** 标题 — **即时击球事件**
  - 水平排列的彩色圆点
  - 荧光黄绿色 = 击球，灰色 = 低活动
  - 击球事件通过 WebSocket 实时接收，瞬间出现在时间线末端
  - 支持水平滚动
...
仪表盘采用**混合实时架构**，浏览器同时维护两条独立数据通路：

#### WebSocket 直连（实时通路）

- **连接目标**：`ws://192.168.4.1:81`（与 ATOM S3 设备直连，绕过 Observer 后端）
- **连接时机**：仪表盘页面加载完成后立即建立连接
- **数据格式**：接收 ATOM S3 发送的 JSON 帧（与 observer.py 接收的格式相同）
- **驱动的 UI 组件**：
  - 3D 线框球体旋转（四元数 qw/qx/qy/qz）
  - RPM 半圆弧仪表数值（rpm 字段）
  - 滚动 RPM 折线图（每帧追加数据点）
  - 击球时间线（收到 event="shot" 帧时即时添加圆点）

#### 连接状态指示器

- 导航栏右侧显示连接状态圆点：
  - **绿色圆点** = WebSocket 已连接，正在接收实时数据
  - **红色圆点** = WebSocket 已断开，实时数据不可用
- 状态变化时平滑过渡动画

#### 数据流处理

```
ATOM S3 (ws://192.168.4.1:81)
    │
    │  WebSocket JSON 帧（50Hz）
    ▼
浏览器 JavaScript
    │
    ├──► 解析 JSON
    ├──► 更新球体四元数 → Canvas 重绘 3D 网球
    ├──► 更新 RPM 数值 → SVG 仪表弧重绘
    ├──► 追加 RPM 到滚动数组 → Canvas 重绘折线图
    └──► 若 event="shot" → 添加击球圆点到时间线
```

#### 降级回退策略

- 若 WebSocket 连接失败（设备未开机、不在同一子网等），仪表盘自动降级为 REST API 轮询模式（每 2 秒从 Observer 后端拉取最新数据），保持基本可用性
- 降级时连接状态指示器显示红色圆点，3D 球体静止，折线图暂停滚动
- WebSocket 断开后每 3 秒自动重试连接，恢复后立刻切回实时模式

#### REST API（历史通路）

- **基地址**：`http://localhost:8080/api/`
- **用途**：会话历史列表、击球详情查询、旋转分析统计、CSV/JSON 导出
- 不受 WebSocket 连接状态影响，只要 Observer 后端运行即可正常工作

#### 滚动 RPM 折线图技术细节

- 渲染方式：Canvas 2D
- 数据缓冲：环形数组，容量 250 个数据点
- 刷新频率：与 WebSocket 数据帧同步（约 50Hz）
- 线条样式：荧光黄绿色（lime / #C8D820），线宽 2px
- 填充区域：线条下方至 X 轴，渐变填充（顶部 rgba(200,216,32,0.3) → 底部透明）
- 自动滚动：新数据从右侧进入，旧数据从左侧移出
...
| 内存占用 | < 50MB |
| 自动重连时间 | < 5 秒 |
| 仪表盘实时更新延迟 | < 50 毫秒（直连 WebSocket，无中间轮询） |
